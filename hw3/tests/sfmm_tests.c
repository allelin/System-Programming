...
// }
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {
    // Checks if memalign if align is a power of 2
    void *p = sf_memalign(32, 6);
    int errno_expected = EINVAL;
    cr_assert_eq(sf_errno, errno_expected, "sf_errno should be set to EINVAL if align is not a power of 2.");
    cr_assert_null(p, "p should be NULL if align is not a power of 2.");
}

Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT) {
    // Checks if malloc is returns NULL if size is too big
    void *p = sf_malloc(5000 * 1000);
    int errno_expected = ENOMEM;
    cr_assert_eq(sf_errno, errno_expected, "sf_errno should be set to ENOMEM if size is too big.");
    cr_assert_null(p, "p should be NULL if size is too big.");
}

Test(sfmm_student_suite, student_test_3, .timeout = TEST_TIMEOUT) {
    // Checks if mallocing returns a splitter
    void *p = sf_malloc(4030);
    int errno_expected = 0;
    int size_expected = 4056;
    cr_assert_eq(sf_errno, errno_expected, "sf_errno should be set to 0 if malloc is successful.");
    cr_assert_not_null(p, "p should not be NULL if malloc is successful.");
    cr_assert(size_expected == size_expected, "there should be no splitter");
}

Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT) {
    // Checks if realloc has a valid pointer
    void *p = (void *)14;
    void *q = sf_realloc(p, 32);
    int errno_expected = EINVAL;
    cr_assert_eq(sf_errno, errno_expected, "sf_errno should be set to EINVAL if ptr is not valid.");
    cr_assert_null(q, "q should be NULL if ptr is not valid.");
}

Test(sfmm_student_suite, student_test_5, .timeout = TEST_TIMEOUT) {
    // Checks if it can malloc and free a page and more
    void *p = sf_malloc(4048);
    sf_errno = 0;
    cr_assert_not_null(p, "p should not be NULL if malloc is successful.");
    assert_free_block_count(0, 0);
    void *q = sf_malloc(2000);
    cr_assert_not_null(q, "q should not be NULL if malloc is successful.");
    assert_free_block_count(0, 1);
    sf_free(p);
    assert_free_block_count(0, 2);
    sf_free(q);
