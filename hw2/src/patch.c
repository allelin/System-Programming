...
void get_some_switches() {
    ...
    while (1) {
        ...
        static struct option long_options[] = {
            ...
            {0, 0, 0, 0}};
        c = getopt_long(Argc, Argv, "-b:cd:D:elno:pr:Rsx:", long_options, &option_index);
        if (c == -1)
            break;
        switch (c) {
            case 1:
...
bool similar(register char *a, register char *b, register int len) {
    while (len) {
...
char *ifetch(register LINENUM line, int whichbuf)
